...
df = pd.read_csv(cwd+'/park-n-go/public/names_copy.csv')
newdf = df.copy()

for i in range(len(df)):
    lat, long = SVY21.computeLatLon(computer, df["y_coord"][i], df["x_coord"][i])
    newdf.loc[i, "x_coord"] = lat
    newdf.loc[i, "y_coord"] = long

print(newdf)

newdf.rename(columns={'x_coord':'latitude', 'y_coord':'longitude'}, inplace=True)
newdf.to_csv(cwd+'/park-n-go/public/names_changed.csv', index = False)
